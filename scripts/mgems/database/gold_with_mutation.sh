...
mkdir -p "${MGEMS_GOLD_STANDARD_MUT_REF_DIR}"
cd "${MGEMS_GOLD_STANDARD_MUT_REF_DIR}" || exit 1
echo "Working in ${MGEMS_GOLD_STANDARD_MUT_REF_DIR}"
...
nr_lines=$(wc -l ref_info.tsv | awk '{ print $1 }')
sed '1d' "${DATA_DIR}/mutated_gold_standard/seed_12345/index.tsv" | cut -f4,6 | awk -v offset="${nr_lines}" -v OFS='\t' '{print $1,$2,NR+offset-2}' >> ref_info.tsv
...
mkdir __tmp
themisto build -k 31 -i themisto_ref_paths.txt -o "${MGEMS_GOLD_STANDARD_MUT_REF_INDEX}" --temp-dir __tmp
rm -rf __tmp


# Create mSWEEP aux file (list of cluster IDs --- one per gold standard. Not running poppunk here)
sed '1d' ref_info.tsv | cut -f3 > "${MGEMS_GOLD_STANDARD_MUT_REF_CLUSTER}"
