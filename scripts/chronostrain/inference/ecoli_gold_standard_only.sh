...
pipeline_single_sample() {
  sample_id=$1
  outdir="${DATA_DIR}/inference/output/chronostrain_gold_standard_only/sample_${sample_id}"
  logdir=${outdir}/logs
  ...
  if [ -f "${filter_breadcrumb}" ]; then
    ...
    env JAX_PLATFORM_NAME=cpu \
      CHRONOSTRAIN_DB_JSON="${CHRONOSTRAIN_GOLD_STANDARD_JSON}" \
...
for i in $(seq 0 99); do
  echo "[* ChronoStrain:ecoli_gold_standard_only.sh] Handling sample ${i}..."
